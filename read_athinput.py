...
def find_match(basedir, patterns):
    glob_match = lambda p: sorted(glob.glob(osp.join(basedir, *p)))
    for p in patterns:
        f = glob_match(p)
        if f:
            break
...
def find_files_vtk2d(basedir, fmt_all):
    ...
    for fmt in fmt_all:
        ...
        files_ = find_match(basedir, patterns)
        if files_:
            files[f'{fmt}'] = files_
            nums[f'{fmt}'] = [int(osp.basename(f).split('.')[1]) \
                              for f in files[f'{fmt}']]
