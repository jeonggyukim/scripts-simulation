...
# Wrapper for join_vtk_partial program
...
join_vtk_c="$ATHENAVTK_DIR/join_vtk_partial.c"
join_vtk=${join_vtk_c%.*}
...
f=( $(find ${indir} -name "*.vtk") ) ; f=${f[0]} ; f=${f##*/}
basename=${f%%.*}
...
do
    ...
    #vtkfiles=( $(find ${indir}/* -mindepth 1 -name "$filepattern") )
    vtkfiles=( $(find ${indir}/* -name "$filepattern") )
    # echo $num
    # echo $filepattern
    echo ${vtkfiles[@]}
